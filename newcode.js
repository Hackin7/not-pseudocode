var InterpreterCode = `### Reserved Keywords ################
# Data Types
INTEGER = 'INTEGER'
FLOAT = 'FLOAT'
REAL = 'REAL'
...
class Lexer(object):
    ...
    def number(self):
        """
        Return a (multidigit) number consumed 
        """
        result = ''
...
class Interpreter:
    ...
    def visit(self, node=None, setVal=None):
        ...
        ### Declaration & Assignment ##################################################################################
            ...
            if node.given_type.name == "ARRAY":
                if node.size2 != None:
                    value = [ [ None for j in range(self.visit(node.size2) + 1) ] for i in range(self.visit(node.size1) + 1) ]
                else:
                    value = [ None for i in range(self.visit(node.size1) + 1) ]
            else:
                value = None
